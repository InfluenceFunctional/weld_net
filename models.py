...
class GatedPixelCNN(nn.Module):  # Dense or residual, gated, blocked, dilated PixelCNN with batchnorm
    def __init__(self, configs, dataDims):
        ...
        if configs.fc_norm is None:
            ...
            self.fc_norm = nn.BatchNorm2d(fc_filters)
        else:
    ...
    def forward(self, input):
        # clean input
        if input.shape[1] > 1:
            conditions = input[:,1:,:,:] # assume global conditions
            input = input[:,:1,:,:] # extract the image part as the regular input
        ...
        if self.do_conditioning:
            conditional_input = self.conditioning_fc_2(F.gelu(self.conditioning_fc_1(conditions)))
